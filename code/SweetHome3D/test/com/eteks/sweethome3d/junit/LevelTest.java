...
public class LevelTest extends ComponentTestFixture {
  public void testLevels() throws RecorderException, NoSuchFieldException, IllegalAccessException, InterruptedException, ComponentSearchException, URISyntaxException {
    ...
    Vector2D[] firstRoomPoints = firstRoom.getPoints();
    firstRoomPoints = new Vector2D[] {firstRoomPoints [0], firstRoomPoints [1], firstRoomPoints [2], firstRoomPoints [firstRoomPoints.length - 1]};
    for (Vector2D point : firstRoomPoints) {
      p = new Point(planView.convertXModelToScreen(point.x) + 1, planView.convertYModelToScreen(point.y) + 1);
      SwingUtilities.convertPointFromScreen(p, planViewComponent);
      tester.actionClick(planViewComponent, new ComponentLocation(p));      
    }
    ...
    Vector2D[] points = newRoom.getPoints();
    for (int i = 0; i < firstRoomPoints.length; i++) {
      assertTrue("Incorrect X [" + i + "] "  + firstRoomPoints [i].x + " " + points [i].x, 
          Math.abs(firstRoomPoints [i].x - points [i].x) < 1E-4);
      assertTrue("Incorrect Y [" + i + "] " + firstRoomPoints [i].y + " " + points [i].y, 
          Math.abs(firstRoomPoints [i].y - points [i].y) < 1E-4);
    }
