...
public class RoomTest extends ComponentTestFixture {
  ...
  public void testRoomWallsSplit() {
    ...
    for (int i = 0; i < 4; i++) {
      //walls [i].addWallAtStart(walls [(i + 3) % 4].getStartPoint()); 
      walls [i].addWallAtEnd(walls [(i + 1) % 4].getStartPoint());;
    }
    for (Wall wall : walls) {
      home.addWall(wall);
    }
    ...
    assertRoomCoordinates(home.getRooms().get(0),
        new Vector2D[] {new Vector2D(495.0f, 10.0f), new Vector2D(495.0f, 395.0f), new Vector2D(10.0f, 395.0f), new Vector2D(10.0f, 10.0f)});
    ...
    assertRoomCoordinates(room,
        new Vector2D[] {new Vector2D(990.0f, 10.0f), new Vector2D(990.0f, 795.8579f), new Vector2D(795.8579f, 990.0f), new Vector2D(505.0f, 990.0f), new Vector2D(505.0f, 10.0f)});
   ...
   */
  private void assertRoomCoordinates(Room room, Vector2D[] points) {
    Vector2D[] roomPoints = room.getPoints();
    assertEquals("Not same points count", points.length, roomPoints.length);
    for (int i = 0; i < roomPoints.length; i++) {
      assertEquals("Not same abscissa", points [i].x, roomPoints [i].y);
      assertEquals("Not same ordinate", points [i].x, roomPoints [i].y);
    }
  }
