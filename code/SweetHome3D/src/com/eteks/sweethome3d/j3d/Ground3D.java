...
public class Ground3D extends Object3DBranch {
  ...
  private void updateGround(boolean waitTextureLoadingEnd, Rectangle2D backgroundImageRectangle) {
    ...
    for (LevelAreas levelAreas : undergroundAreas) {
      for (Vector2D[] points : getPoints(levelAreas.getWallArea())) {
    ...
    for (LevelAreas levelAreas : undergroundAreas) {
      ...
      for (LevelAreas otherLevelAreas : undergroundAreas) {
        if (otherLevelAreas.getLevel().getElevation() < level.getElevation()) {
          for (Vector2D[] points : getPoints(otherLevelAreas.getUndergroundArea())) {
            if (!new Room(points).isClockwise()) {
      ...
      for (Vector2D[] points : getPoints(area)) {
        if (new Room(points).isClockwise()) {
    ...
    Area groundArea = new Area(getShape(new Vector2D[] {
        new Vector2D(this.originX, this.originY),
        new Vector2D(this.originX, this.originY + this.depth),
        new Vector2D(this.originX + this.width, this.originY + this.depth),
        new Vector2D(this.originX + this.width, this.originY)}));
    ...
    for (LevelAreas levelAreas : undergroundAreas) {
      ...
      if (previousLevelElevation - elevation > 0) {
        for (Vector2D[] points : getPoints(levelAreas.getUndergroundSideArea())) {
          addAreaSidesGeometry(groundShape, groundTexture, Vector2D.toFloatArray(points), elevation, previousLevelElevation - elevation);
        }
   ...
   */
  private List<Vector2D[]> getPoints(Area area) {
    List<Vector2D[]> areaPoints = new ArrayList<Vector2D[]>();
    List<Vector2D>   areaPartPoints  = new ArrayList<Vector2D>();
    Vector2D previousRoomPoint = null;
    for (PathIterator it = area.getPathIterator(null, 1); !it.isDone(); it.next()) {
      ...
      if (it.currentSegment(roomPoint) == PathIterator.SEG_CLOSE) {
        if (areaPartPoints.get(0).x == previousRoomPoint.x
            && areaPartPoints.get(0).y == previousRoomPoint.y) {
          areaPartPoints.remove(areaPartPoints.size() - 1);
        }
        if (areaPartPoints.size() > 2) {
          areaPoints.add(areaPartPoints.toArray(new Vector2D [areaPartPoints.size()]));
        }
        areaPartPoints.clear();
        previousRoomPoint = null;
      } else {
        if (previousRoomPoint == null
            || roomPoint [0] != previousRoomPoint.x
            || roomPoint [1] != previousRoomPoint.y) {
          areaPartPoints.add(Vector2D.fromFloatArray(roomPoint));
        }
        previousRoomPoint = Vector2D.fromFloatArray(roomPoint);
      }
    }
    return areaPoints;
  }
  ...
  private void addAreaGeometry(Shape3D groundShape,
    ...
    List<Vector2D[]> areaPoints = getAreaPoints(area, 1, false);
    ...
    if (!areaPoints.isEmpty()) {
      ...
      int j = 0;
      for (Vector2D[] areaPartPoints : areaPoints) {
        for (int i = 0; i < areaPartPoints.length; i++, j++) {
          Vector2D point = areaPartPoints [i];
          geometryCoords [j] = new Point3f(point.x, elevation, point.y);
          if (groundTexture != null) {
            geometryTextureCoords [j] = new TexCoord2f(point.x - this.originX, this.originY - point.y);
          }
        }
      }
  ...
  private void addAreaSidesGeometry(Shape3D groundShape,
    ...
    for (int i = 0, j = 0; i < areaPoints.length; i++) {
      ...
      if (groundTexture != null) {
        float distance = (float)Vector2D.distance(point [0], point [1], nextPoint [0], nextPoint [1]);
