...
public abstract class Object3DBranch extends BranchGroup {
   ...
   */
  protected Shape getShape(Vector2D[] points) {
    return ShapeTools.getShape(Vector2D.toFloatArray(points), true, null);
  }
   ...
   */
  protected List<Vector2D[]> getAreaPoints(Area area,
                                           float flatness,
                                           boolean reversed) {
   ...
   */
  protected List<Vector2D[]> getAreaPoints(Area area,
                                           List<Vector2D[]> areaPoints,
                                           List<Vector2D[]> areaHoles,
                                           float flatness,
                                           boolean reversed) {
    List<List<Vector2D>> areaPointsLists = new LinkedList<List<Vector2D>>();
    List<List<Vector2D>> areaHolesLists = new LinkedList<List<Vector2D>>();
    ArrayList<Vector2D>  currentPathPoints = null;
    Vector2D previousPoint = null;
    for (PathIterator it = area.getPathIterator(null, flatness); !it.isDone(); it.next()) {
      float [] point = new float [2];
      switch (it.currentSegment(point)) {
        case PathIterator.SEG_MOVETO :
          currentPathPoints = new ArrayList<Vector2D>();
          currentPathPoints.add(Vector2D.fromFloatArray(point));
          previousPoint = Vector2D.fromFloatArray(point);
          break;
        case PathIterator.SEG_LINETO :
          if (point [0] != previousPoint.x
              || point [1] != previousPoint.y) {
            currentPathPoints.add(Vector2D.fromFloatArray(point));
          }
          previousPoint = Vector2D.fromFloatArray(point);
          break;
        case PathIterator.SEG_CLOSE:
          Vector2D firstPoint = currentPathPoints.get(0);
          if (firstPoint.x == previousPoint.x
              && firstPoint.y == previousPoint.y) {
            currentPathPoints.remove(currentPathPoints.size() - 1);
          }
          if (currentPathPoints.size() > 2) {
            Vector2D[] areaPartPoints = currentPathPoints.toArray(new Vector2D [currentPathPoints.size()]);
            ...
            if (subRoom.getArea() > 0) {
              ...
              if (areaPoints != null || areaHoles != null) {
                ...
                if (pathPointsClockwise ^ reversed) {
                  currentPathPoints = (ArrayList<Vector2D>)currentPathPoints.clone();
    ...
    List<Vector2D[]> areaPointsWithoutHoles = new ArrayList<Vector2D[]>();
    if (areaHolesLists.isEmpty() && areaPoints != null) {
      ...
      List<List<Vector2D>> sortedAreaPoints;
      Map<List<Vector2D>, Area> subAreas = new HashMap<List<Vector2D>, Area>(areaPointsLists.size());
      if (areaPointsLists.size() > 1) {
        sortedAreaPoints = new ArrayList<List<Vector2D>>(areaPointsLists.size());
        for (int i = 0; !areaPointsLists.isEmpty(); ) {
          List<Vector2D> testedArea = areaPointsLists.get(i);
          int j = 0;
          for ( ; j < areaPointsLists.size(); j++) {
            if (i != j) {
              List<Vector2D> testedAreaPoints = areaPointsLists.get(j);
              Area subArea = subAreas.get(testedAreaPoints);
              if (subArea == null) {
                subArea = new Area(getShape((Vector2D[])testedAreaPoints.toArray()));
                // Store computed area for future reuse
                subAreas.put(testedAreaPoints, subArea);
              }
              if (subArea.contains(testedArea.get(0).x, testedArea.get(0).y)) {
                break;
              }
            }
          }
      ...
      for (int i = sortedAreaPoints.size() - 1; i >= 0; i--) {
        List<Vector2D> enclosingAreaPartPoints = sortedAreaPoints.get(i);
        Area subArea = subAreas.get(enclosingAreaPartPoints);
        if (subArea == null) {
          subArea = new Area(getShape(enclosingAreaPartPoints.toArray(new Vector2D[enclosingAreaPartPoints.size()])));
          // No need to store computed area because it won't be reused
        }
        List<List<Vector2D>> holesInArea = new ArrayList<List<Vector2D>>();
        // Search the holes contained in the current area part
        for (List<Vector2D> holePoints : areaHolesLists) {
          if (subArea.contains(holePoints.get(0).x, holePoints.get(0).y)) {
            holesInArea.add(holePoints);
          }
        }

        Vector2D lastEnclosingAreaPointJoiningHoles = null;
        while (!holesInArea.isEmpty()) {
          ...
          for (int j = 0; j < holesInArea.size() && minDistance > 0; j++) {
            List<Vector2D> holePoints = holesInArea.get(j);
            for (int k = 0; k < holePoints.size() && minDistance > 0; k++) {
              for (int l = 0; l < enclosingAreaPartPoints.size() && minDistance > 0; l++) {
                Vector2D enclosingAreaPartPoint = enclosingAreaPartPoints.get(l);
                float distance = (float)Point2D.distanceSq(holePoints.get(k).x, holePoints.get(k).y,
                    enclosingAreaPartPoint.x, enclosingAreaPartPoint.y);
          ...
          List<Vector2D> closestHolePoints = holesInArea.get(closestHolePointsIndex);
          if (minDistance != 0) {
          ...
          List<Vector2D> lastPartPoints = closestHolePoints.subList(closestPointIndex, closestHolePoints.size());
      ...
      for (List<Vector2D> pathPoints : sortedAreaPoints) {
        if (reversed) {
          Collections.reverse(pathPoints);
        }
        areaPointsWithoutHoles.add(pathPoints.toArray(new Vector2D [pathPoints.size()]));
      }
    }
