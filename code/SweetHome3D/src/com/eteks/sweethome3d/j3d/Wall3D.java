...
public class Wall3D extends Object3DBranch {
  ...
  private void createWallGeometries(List<Geometry> bottomGeometries,
    ...
    final Vector2D[] wallSidePoints = getWallSidePoints(wallSide);
    Shape wallSideShape = getShape(wallSidePoints);
    final Vector2D[] wallSideOrBaseboardPoints = baseboard == null
        ? wallSidePoints
    ...
    final Vector2D textureReferencePoint = wallSide == WALL_LEFT_SIDE
    ...
    List<Vector2D> points = new ArrayList<Vector2D>(4);
    ...
    for (PathIterator it = wallSideOrBaseboardArea.getPathIterator(null); !it.isDone(); it.next()) {
      ...
      if (it.currentSegment(wallPoint) == PathIterator.SEG_CLOSE) {
        if (points.size() > 2) {
          ...
          if (points.size() > 2) {
            Vector2D[] wallPartPoints = points.toArray(new Vector2D[points.size()]);
        ...
        points.add(Vector2D.fromFloatArray(wallPoint));
          ...
          if (it.currentSegment(wallPoint) == PathIterator.SEG_CLOSE) {
            ...
            if (points.size() > 2) {
              ...
              for (int i = 0; i < wallPartPoints.length; i++) {
                double xTopPointWithZeroYaw = cosWallYawAngle * wallPartPoints[i].x + sinWallYawAngle * wallPartPoints[i].y;
                ...
                if (doorOrWindowTop > topPointWithZeroYawElevation) {
                  ...
                  wallPartPoints [i].x += (float)(translation * cosWallYawAngle);
                  wallPartPoints [i].y += (float)(translation * sinWallYawAngle);
                }
              }
   ...
   */
  private Vector2D[] getWallSidePoints(int wallSide) {
    Wall wall = (Wall)getUserData();
    Vector2D[] wallPoints = wall.getPoints();

    if (wallSide == WALL_LEFT_SIDE) {
      for (int i = wallPoints.length / 2; i < wallPoints.length; i++) {
        wallPoints [i].x = (wallPoints [i].x + wallPoints [wallPoints.length - i - 1].x) / 2;
        wallPoints [i].y = (wallPoints [i].y + wallPoints [wallPoints.length - i - 1].y) / 2;
      }
   ...
   */
  private Vector2D[] getWallBaseboardPoints(int wallSide) {
    Wall wall = (Wall)getUserData();
    Vector2D[] wallPointsIncludingBaseboards = wall.getPoints(true);
    Vector2D[] wallPoints = wall.getPoints();
  ...
  private Geometry createVerticalPartGeometry(Wall wall,
                                              Vector2D[] points, float minElevation,
                                              ...
                                              Vector2D textureReferencePoint,
                                              int wallSide) {
    ...
    if ((arcExtent == null || arcExtent == 0)
      ...
      List<Vector2D> pointsList = new ArrayList<Vector2D>(points.length * 2);
      pointsList.add(points [0]);
      for (int i = 1; i < points.length; i++) {
        double distance = Point2D.distance(points [i - 1].x, points [i - 1].y, points [i].x, points [i].y) - subpartSize / 2;
        double angle = Math.atan2(points [i].y - points [i - 1].y, points [i].x - points [i - 1].x);
        double cosAngle = Math.cos(angle);
        double sinAngle = Math.sin(angle);
        for (double d = 0; d < distance; d += subpartSize) {
          pointsList.add(new Vector2D ((float)(points [i - 1].x + d * cosAngle), (float)(points [i - 1].y + d * sinAngle)));
        }
        pointsList.add(points [i]);
      }
      points = pointsList.toArray(new Vector2D [pointsList.size()]);
    }
    ...
    Vector2D arcCircleCenter = null;
    float arcCircleRadius = 0;
    float referencePointAngle = 0;
    if (arcExtent != null && arcExtent != 0) {
      arcCircleCenter = wall.getArcCircleCenter();
      arcCircleRadius = (float)Point2D.distance(arcCircleCenter.x, arcCircleCenter.y,
          xStart, yStart);
      referencePointAngle = textureReferencePoint.getAngle(arcCircleCenter);
    }
    for (int i = 0; i < points.length; i++) {
      bottom [i] = new Point3f(points [i].x, minElevation, points [i].y);
      // Compute vertical top point
      float topY = getWallPointElevation(points [i].x, points [i].y, cosWallYawAngle, sinWallYawAngle, topLineAlpha, topLineBeta);
      top [i] = new Point3f(points [i].x, topY, points [i].y);
    }
    ...
    for (int i = 0; i < points.length; i++) {
      if (arcCircleCenter == null) {
        ...
        distanceSqToWallMiddle [i] = arcCircleRadius
            - arcCircleCenter.distance(bottom [i].x, bottom [i].z);
      ...
      for (int index = 0; index < points.length; index++) {
        if (usedRectangle [index]) {
          ...
          if (Math.abs(distanceSqToWallMiddle [index] - halfThicknessSq) < epsilon
            ...
            if (arcCircleCenter == null) {
              firstHorizontalTextureCoords = (float)textureReferencePoint.distance(points [index]);
              secondHorizontalTextureCoords = (float)textureReferencePoint.distance(points [nextIndex]);
            } else {
              if (pointUCoordinates [index] == null) {
                float pointAngle = points [index].getAngle(arcCircleCenter);
              ...
              if (pointUCoordinates [nextIndex] == null) {
                float pointAngle = points [nextIndex].getAngle(arcCircleCenter);
                pointAngle = adjustAngleOnReferencePointAngle(pointAngle, referencePointAngle, arcExtent);
            ...
            float horizontalTextureCoords = (float)points [index].distance(points [nextIndex]);
   ...
   */
  private Geometry createHorizontalPartGeometry(Vector2D[] points, float y,
                                                boolean reverseOrder, boolean roundWall) {
    Point3f [] coords = new Point3f [points.length];
    for (int i = 0; i < points.length; i++) {
      coords [i] = new Point3f(points [i].x, y, points [i].y);
    }
   ...
   */
  private Geometry createTopPartGeometry(Vector2D[] points,
                                         double cosWallYawAngle, double sinWallYawAngle,
                                         double topLineAlpha, double topLineBeta,
                                         boolean roundWall) {
    Point3f [] coords = new Point3f [points.length];
    for (int i = 0; i < points.length; i++) {
      double xTopPointWithZeroYaw = cosWallYawAngle * points [i].x + sinWallYawAngle * points [i].y;
      float topY = (float)(topLineAlpha * xTopPointWithZeroYaw + topLineBeta);
      coords [i] = new Point3f(points [i].x, topY, points [i].y);
    }
    GeometryInfo geometryInfo = new GeometryInfo(GeometryInfo.POLYGON_ARRAY);
    geometryInfo.setCoordinates (coords);
    geometryInfo.setStripCounts(new int [] {coords.length});
  ...
  private void createGeometriesSurroundingDoorOrWindow(HomeDoorOrWindow doorOrWindow,
                                                       ...
                                                       Vector2D[] wallSidePoints,
                                                       float wallElevation,
                                                       double cosWallYawAngle, double sinWallYawAngle,
                                                       double topLineAlpha, double topLineBeta,
                                                       HomeTexture texture, Vector2D textureReferencePoint,
                                                       int wallSide) {
    ...
    if (!fullFaceArea.isEmpty()) {
      ...
      Vector2D wallFirstPoint = wallSide == WALL_LEFT_SIDE
          ? wallSidePoints [0]
          : wallSidePoints [wallSidePoints.length - 1];
      Vector2D wallSecondPoint = wallSide == WALL_LEFT_SIDE
          ? wallSidePoints [wallSidePoints.length / 2 - 1]
          : wallSidePoints [wallSidePoints.length / 2];
      float frontSideToWallDistance = wallFirstPoint.x != wallSecondPoint.x || wallFirstPoint.y != wallSecondPoint.y
          ? (float)Line2D.ptLineDist(wallFirstPoint.x, wallFirstPoint.y, wallSecondPoint.x, wallSecondPoint.y, xPieceSide, yPieceSide)
          : (float)Point2D.distance(wallFirstPoint.x, wallFirstPoint.y, xPieceSide, yPieceSide);
      float position = (float)Line2D.relativeCCW(wallFirstPoint.x, wallFirstPoint.y,
          wallSecondPoint.x, wallSecondPoint.y, xPieceSide, yPieceSide);
      ...
      Point3f wallPoint = new Point3f(wallFirstPoint.x, wallElevation, wallFirstPoint.y);
      invertedFrontAreaTransform.transform(wallPoint);
      wallPath.moveTo(wallPoint.x, wallPoint.y);
      wallPoint = new Point3f(wallSecondPoint.x, wallElevation, wallSecondPoint.y);
      invertedFrontAreaTransform.transform(wallPoint);
      wallPath.lineTo(wallPoint.x, wallPoint.y);
      Point3f topWallPoint1 = new Point3f(wallSecondPoint.x,
          getWallPointElevation(wallSecondPoint.x, wallSecondPoint.y,
              cosWallYawAngle, sinWallYawAngle, topLineAlpha, topLineBeta), wallSecondPoint.y);
      invertedFrontAreaTransform.transform(topWallPoint1);
      wallPath.lineTo(topWallPoint1.x, topWallPoint1.y);
      Point3f topWallPoint2 = new Point3f(wallFirstPoint.x,
          getWallPointElevation(wallFirstPoint.x, wallFirstPoint.y,
              cosWallYawAngle, sinWallYawAngle, topLineAlpha, topLineBeta), wallFirstPoint.y);
      ...
      if (!doorOrWindowSurroundingArea.isEmpty()) {
        ...
        List<Vector2D[]> doorOrWindowSurroundingAreasPoints = getAreaPoints(doorOrWindowSurroundingArea, flatness, reversed);
        if (!doorOrWindowSurroundingAreasPoints.isEmpty()) {
          ...
          for (int i = 0; i < doorOrWindowSurroundingAreasPoints.size(); i++) {
            Vector2D[] areaPoints = doorOrWindowSurroundingAreasPoints.get(i);
          ...
          int i = 0;
          for (Vector2D[] areaPoints : doorOrWindowSurroundingAreasPoints) {
            Point3f point = new Point3f(areaPoints [0].x, areaPoints [0].y, 0);
            ...
            if (texture != null) {
              ...
              float horizontalTextureCoords = (float)Point2D.distance(textureReferencePoint.x, textureReferencePoint.y,
                  point.x, point.z);
              if (wallSide == WALL_LEFT_SIDE && texture.isLeftToRightOriented()) {
            ...
            for (int j = 0; j < areaPoints.length; j++, i++) {
              ...
              double nextDistanceToTop = Line2D.ptLineDistSq(topWallPoint1.x, topWallPoint1.y, topWallPoint2.x, topWallPoint2.y,
                  areaPoints [nextPointIndex].x, areaPoints [nextPointIndex].y);
              ...
              Point3f nextPoint = new Point3f(areaPoints [nextPointIndex].x, areaPoints [nextPointIndex].y, 0);
