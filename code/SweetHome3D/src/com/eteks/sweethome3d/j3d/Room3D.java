...
public class Room3D extends Object3DBranch {
  ...
  private Geometry [] createRoomGeometries(int roomPart, HomeTexture texture) {
    ...
    if ((roomPart == FLOOR_PART && room.isFloorVisible()
      ...
      if (roomPart == CEILING_PART
        ...
        float firstPointElevation = getRoomHeightAt(points [0].x, points [0].y);
        for (int i = 1; i < points.length && sameElevation; i++) {
          sameElevation = getRoomHeightAt(points [i].x, points [i].y) == firstPointElevation;
        }
      }

      // Retrieve room points
      List<Vector2D[]> roomPoints;
      List<Vector2D[]> roomHoles;
      List<Vector2D[]> roomPointsWithoutHoles;
      ...
      if (!room.isSingular()
        ...
        roomPoints = new ArrayList<Vector2D[]>();
        roomHoles = new ArrayList<Vector2D[]>();
        ...
        roomPoints = Arrays.asList(new Vector2D[][] {points});
      ...
      if (!roomPointsWithoutHoles.isEmpty()) {
        ...
        for (int i = 0; i < roomPointsWithoutHoles.size(); i++) {
          Vector2D[] roomPartPoints = roomPointsWithoutHoles.get(i);
          ...
          for (int j = 0; j < roomPartPoints.length; j++) {
            roomPartPointElevations [j] = roomPart == FLOOR_PART
                ? roomElevation
                : getRoomHeightAt(roomPartPoints [j].x, roomPartPoints [j].y);
        ...
        if (roomAtSameElevation && subpartSize > 0) {
          for (int i = 0; i < roomPointsWithoutHoles.size(); i++) {
            ...
            for (Vector2D point : roomPartPoints) {
              xMin = Math.min(xMin, point.x);
              xMax = Math.max(xMax, point.x);
              zMin = Math.min(zMin, point.y);
              zMax = Math.max(zMax, point.y);
            }
            ...
            for (float xSquare = xMin; xSquare < xMax; xSquare += subpartSize) {
              for (float zSquare = zMin; zSquare < zMax; zSquare += subpartSize) {
                ...
                if (!roomPartSquare.isEmpty()) {
                  List<Vector2D[]> geometryPartPointsWithoutHoles =
      ...
      if (floorBottomVisible) {
        List<Vector2D[]> floorBottomPointsWithoutHoles;
        if (roomVisibleArea != null
          ...
          floorBottomPointsWithoutHoles = Arrays.asList(new Vector2D[][] {getReversedArray(points)});
        }
        ...
        if (!floorBottomPointsWithoutHoles.isEmpty()) {
          ...
          if (subpartSize > 0) {
            for (int i = 0 ; i < floorBottomPointsWithoutHoles.size(); i++) {
              Vector2D[] floorBottomPartPoints = floorBottomPointsWithoutHoles.get(i);
              float xMin = Float.MAX_VALUE;
              float xMax = Float.MIN_VALUE;
              float zMin = Float.MAX_VALUE;
              float zMax = Float.MIN_VALUE;
              for (Vector2D point : floorBottomPartPoints) {
                xMin = Math.min(xMin, point.x);
                xMax = Math.max(xMax, point.x);
                zMin = Math.min(zMin, point.y);
                zMax = Math.max(zMax, point.y);
              ...
              for (float xSquare = xMin; xSquare < xMax; xSquare += subpartSize) {
                for (float zSquare = zMin; zSquare < zMax; zSquare += subpartSize) {
                  ...
                  if (!floorBottomPartSquare.isEmpty()) {
                    List<Vector2D[]> geometryPartPointsWithoutHoles = getAreaPoints(floorBottomPartSquare, 1, true);
                    if (!geometryPartPointsWithoutHoles.isEmpty()) {
                      geometries.add(computeRoomPartGeometry(geometryPartPointsWithoutHoles,
   ...
   */
  private Geometry computeRoomPartGeometry(List<Vector2D[]> geometryPoints,
    ...
    for (int i = 0; i < geometryPoints.size(); i++) {
      Vector2D[] areaPoints = geometryPoints.get(i);
    ...
    for (int j = 0; j < geometryPoints.size(); j++) {
      Vector2D[] areaPoints = geometryPoints.get(j);
      ...
      for (int k = 0; k < areaPoints.length; k++) {
        float y = floorBottomPart
            ...
            : (roomPartPointElevations != null
                  ...
                  : roomPartElevation);
        coords [i++] = new Point3f(areaPoints [k].x, y, areaPoints [k].y);
      }
    }
    ...
    if (texture != null) {
      ...
      for (Vector2D[] areaPoints : geometryPoints) {
        for (int k = 0; k < areaPoints.length; k++) {
          textureCoords [i++] = new TexCoord2f(areaPoints [k].x,
              floorPart
                  ? -areaPoints [k].y
                  : areaPoints [k].y);
        }
   ...
   */
  private Geometry computeRoomBorderGeometry(List<Vector2D[]> geometryRooms,
                                             List<Vector2D[]> geometryHoles,
                                             Level roomLevel, float roomElevation,
                                             HomeTexture texture) {
    int vertexCount = 0;
    for (Vector2D[] geometryPoints : geometryRooms) {
      vertexCount += geometryPoints.length;
    }
    for (Vector2D[] geometryHole : geometryHoles) {
    ...
    for (Vector2D[] geometryPoints : geometryRooms) {
      for (int j = 0; j < geometryPoints.length; j++) {
        coords [i++] = new Point3f(geometryPoints [j].x, roomElevation, geometryPoints [j].y);
        coords [i++] = new Point3f(geometryPoints [j].x, floorBottomElevation, geometryPoints [j].y);
        int nextPoint = j < geometryPoints.length - 1
            ? j + 1
            : 0;
        coords [i++] = new Point3f(geometryPoints [nextPoint].x, floorBottomElevation, geometryPoints [nextPoint].y);
        coords [i++] = new Point3f(geometryPoints [nextPoint].x, roomElevation, geometryPoints [nextPoint].y);
      }
    }
    // Compute holes borders coordinates
    for (Vector2D[] geometryHole : geometryHoles) {
      for (int j = 0; j < geometryHole.length; j++) {
        coords [i++] = new Point3f(geometryHole [j].x, roomElevation, geometryHole [j].y);
        int nextPoint = j < geometryHole.length - 1
            ? j + 1
            : 0;
        coords [i++] = new Point3f(geometryHole [nextPoint].x, roomElevation, geometryHole [nextPoint].y);
        coords [i++] = new Point3f(geometryHole [nextPoint].x, floorBottomElevation, geometryHole [nextPoint].y);
        coords [i++] = new Point3f(geometryHole [j].x, floorBottomElevation, geometryHole [j].y);
      }
    }
    ...
    if (texture != null) {
      ...
      if (texture.isFittingArea()) {
        ...
        for (Vector2D[] geometryPoints : geometryRooms) {
          for (int j = 0; j < geometryPoints.length; j++) {
            textureCoords [i++] =
            textureCoords [i++] = new TexCoord2f(geometryPoints [j].x, -geometryPoints [j].y);
            int nextPoint = j < geometryPoints.length - 1
                ? j + 1
                : 0;
            textureCoords [i++] =
            textureCoords [i++] = new TexCoord2f(geometryPoints [nextPoint].x, -geometryPoints [nextPoint].y);
          }
        }
        // Compute holes borders texture coordinates
        for (Vector2D[] geometryHole : geometryHoles) {
          for (int j = 0; j < geometryHole.length; j++) {
            textureCoords [i] = new TexCoord2f(geometryHole [j].x, -geometryHole [j].y);
            int nextPoint = j < geometryHole.length - 1
                ? j + 1
                : 0;
            textureCoords [i + 1] = new TexCoord2f(geometryHole [nextPoint].x, -geometryHole [nextPoint].y);
        ...
        for (Vector2D[] geometryPoints : geometryRooms) {
          for (int j = 0; j < geometryPoints.length; j++) {
            ...
            int nextPoint = j < geometryPoints.length - 1
                ? j + 1
                : 0;
            float textureCoord = (float)(Point2D.distance(geometryPoints [j].x, geometryPoints [j].y,
                geometryPoints [nextPoint].x, geometryPoints [nextPoint].y));
        ...
        for (Vector2D[] geometryHole : geometryHoles) {
          ...
          textureCoords [i++] = new TexCoord2f(0, 0);
            int nextPoint = j < geometryHole.length - 1
                ? j + 1
                : 0;
            float textureCoord = (float)(Point2D.distance(geometryHole [j].x, geometryHole [j].y,
                geometryHole [nextPoint].x, geometryHole [nextPoint].y));
            textureCoords [i++] = new TexCoord2f(textureCoord, 0);
            textureCoords [i++] = new TexCoord2f(textureCoord, roomLevel.getFloorThickness());
            textureCoords [i++] = new TexCoord2f(0, roomLevel.getFloorThickness());
          }
   ...
   */
  private Vector2D[] getReversedArray(Vector2D[] points) {
    points = points.clone();
    List<Vector2D> pointList = Arrays.asList(points);
  ...
  private float getRoomHeightAt(float x, float y) {
    ...
    if (roomLevel == null || isLastLevel(roomLevel, levels)) {
      if (room.isCeilingFlat()) {
        ...
        Vector2D[] closestWallPoints = null;
        int closestIndex = -1;
        for (Wall wall : this.home.getWalls()) {
          if ((wall.getLevel() == null || wall.getLevel().isViewable())
              && wall.isAtLevel(roomLevel)) {
            Vector2D[] points = wall.getPoints();
            for (int i = 0; i < points.length; i++) {
              double distanceToWallPoint = points [i].distanceSq(x, y);
          ...
          if (closestIndex == 0 || closestIndex == closestWallPoints.length - 1) { // Wall start
            ...
            if (closestWall.isTrapezoidal()) {
              ...
              if (arcExtent == null
                ...
                Vector2D ArcCircleCenter = closestWall.getArcCircleCenter();
                Vector2D ClosestPoint = closestWallPoints [closestIndex];
                double centerToClosestPointDistance = ArcCircleCenter.distance(ClosestPoint);
                Vector2D start = closestWall.getStartPoint().toVector();
                double centerToStartPointDistance = ArcCircleCenter.distance(start);
                Vector2D scalar = ClosestPoint.sub(ArcCircleCenter).tim(start.sub(ArcCircleCenter));
                double scalarProduct = scalar.x + scalar.y;
  ...
  private void updateFilledRoomPartAppearance(final Appearance roomPartAppearance,
    ...
    if (roomPartTexture == null) {
      ...
      if (roomPartTexture.isFittingArea()) {
        roomPartAppearance.setTextureAttributes(getTextureAttributesFittingArea(roomPartTexture, Vector2D.toFloatArray(room.getPoints()), floor));
      } else {
