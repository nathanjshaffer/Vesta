...
public class RoomController implements Controller {
  ...
  protected void updateProperties() {
    ...
    if (wallSides.isEmpty()) {
      ...
      Integer wallSidesColor = firstWallSide.getColor();
      if (wallSidesColor != null) {
        for (int i = 1; i < wallSides.size(); i++) {
          WallSide wallSide = wallSides.get(i);
          if (!wallSidesColor.equals(wallSide.getColor())) {
            wallSidesColor = null;
            break;
          }
        }
      ...
      for (int i = 0; i < wallSides.size(); i++) {
        WallSide wallSide = wallSides.get(i);
        if ((wallSide.getColor()) != null
            || (wallSide.getTexture()) != null) {
          defaultColorsAndTextures = false;
          break;
        }
      }
   ...
   */
  private boolean isRoomItersectingWallSide(Vector2D[] wallPoints, int wallSide, Area roomArea) {
   ...
   */
  private Area getWallSideArea(Vector2D[] wallPoints, int wallSide) {
    ...
    if (wallPoints.length == 4) {
      if (wallSide == WallSide.LEFT_SIDE) {
        return new Area(getPath(new Wall(
            wallPoints [0].x, wallPoints [0].y, wallPoints [1].x, wallPoints [1].y, thickness, 0).getPoints(), true));
      } else {
        return new Area(getPath(new Wall(
            wallPoints [2].x, wallPoints [2].y, wallPoints [3].x, wallPoints [3].y, thickness, 0).getPoints(), true));
      }
    } 
    else {
      Vector2D[] wallSidePoints = new Vector2D [wallPoints.length / 2];
      ...
      for (int i = 0; i < wallSideWalls.length; i++) {
        wallSideWalls [i] = new Wall(wallSidePoints [i].x, wallSidePoints [i].y, wallSidePoints [i + 1].x, wallSidePoints [i + 1].y, thickness, 0);
        if (i > 0) {
          wallSideWalls [i].addWallAtStart(wallSideWalls [i - 1].getEndPoint());
        }
      }
      wallSidePoints = new Vector2D [wallPoints.length];
      Vector2D[] wallSideWallPoints = null;
      for (int i = 0; i < wallSideWalls.length; i++) {
   ...
   */
  private GeneralPath getPath(Vector2D[] points, boolean closedPath) {
    ...
    for (PathIterator it = area.getPathIterator(null); !it.isDone(); ) {
      ...
      switch (it.currentSegment(roomPoint)) {
        ...
        case PathIterator.SEG_CLOSE :
          Vector2D[] pathPoints =
              currentPathPoints.toArray(new Vector2D [currentPathPoints.size()]);
  ...
  private float getSignedSurface(Vector2D areaPoints []) {
    ...
    for (int i = 1; i < areaPoints.length; i++) {
      ...
      area -= areaPoints [i].y * areaPoints [i - 1].x;
    }
    area += areaPoints [0].x * areaPoints [areaPoints.length - 1].y;
    area -= areaPoints [0].y * areaPoints [areaPoints.length - 1].x;
    return area / 2;
  }
  ...
  public void modifyRooms() {
    ...
    if (!selectedRooms.isEmpty()) {
      ...
      // Apply modification
      List<WallState> deletedWalls = new ArrayList<WallState>();
      List<WallState> addedWalls = new ArrayList<WallState>();
      ...
      WallSideState [] modifiedWallSides = new WallSideState [selectedRoomsWallSides.size()];
      for (int i = 0; i < modifiedWallSides.length; i++) {
        modifiedWallSides [i] = new WallSideState(selectedRoomsWallSides.get(i));
      }
      ...
      if (this.undoSupport != null) {
        this.undoSupport.postEdit(new RoomsAndWallSidesModificationUndoableEdit(this.home, this.preferences,
            ...
            wallSidesColor, wallSidesTexture, wallSidesShininess, wallSidesBaseboardVisible,
            wallSidesBaseboardThickness, wallSidesBaseboardHeight, wallSidesBaseboardPaint,
            wallSidesBaseboardColor, wallSidesBaseboardTexture, deletedWalls.toArray(new WallState [deletedWalls.size()]),
            addedWalls.toArray(new WallState [addedWalls.size()])));
      }
      if (name != null) {
  ...
  private boolean splitWalls(List<WallSide> wallSides,
                             List<WallState> deletedWalls,
                             List<WallState> addedWalls,
                             List<Selectable> selectedItems) {
    Map<Wall, WallState> existingWalls = null;
          ...
          for (WallSide intersectedWallSide : wallSides) {
            ...
            if (wall != intersectedWall) {
              ...
              if (!intersectedWallArea.isEmpty()
                ...
                Vector2D intersection = wall.getWallIntersectionPoint(intersectedWall);
                if (intersection != null) {
                  secondWall = wall.split(intersection);
                  secondWall.setLevel(wall.getLevel());

                  if (wall.getLength() > intersectedWall.getThickness() / 2
                    ...
                    if (existingWalls == null) {
                      ...
                      existingWalls = new HashMap<Wall, WallState>(wallSides.size());
                      for (WallSide side : wallSides) {
                        if (!existingWalls.containsKey(side.getWall())) {
                          existingWalls.put(side.getWall(), new WallState(side.getWall()));
                        }
                      }
                    }
                    ...
                    if (wall.getHeightAtEnd() != null) {
                      Float heightAtIntersecion = wall.getHeight()
                          + (wall.getHeightAtEnd() - wall.getHeight())
                            * (float)wall.getStartPoint().toVector().distance( intersection)
                            / wall.getLength();
                      secondWall.setHeight(heightAtIntersecion);
                    }
                    newWalls.add(secondWall);
                    selectedItems.add(secondWall);
                    wallSides.add(secondWall.getSide(wallSide.getSide()));
  ...
  private static class RoomsAndWallSidesModificationUndoableEdit extends LocalizedUndoableEdit {
    ...
    private final WallSideState [] modifiedWallSides;
    ...
    private final WallState []     deletedWalls;
    private final WallState []     addedWalls;
    ...
    private RoomsAndWallSidesModificationUndoableEdit(Home home,
                                          ...
                                          Boolean ceilingFlat,
                                          WallSideState [] modifiedWallSides,
                                          ...
                                          WallState [] deletedWalls,
                                          WallState [] addedWalls) {
  ...
  private static void doModifyRoomsAndWallSides(Home home, ModifiedRoom [] modifiedRooms,
                                                ...
                                                RoomPaint ceilingPaint, Integer ceilingColor, HomeTexture ceilingTexture, Float ceilingShininess, Boolean ceilingFlat, WallSideState [] modifiedWallSides,
                                                float newWallBaseboardThickness,
                                                ...
                                                Integer wallSidesBaseboardColor, HomeTexture wallSidesBaseboardTexture, WallState [] deletedWalls,
                                                WallState [] addedWalls) {
    if (deletedWalls != null) {
      for (WallState newWall : addedWalls) {
    ...
    for (WallSideState modifiedWallSide : modifiedWallSides) {
      WallSide wallSide = modifiedWallSide.getWallSide();
      if (wallSidesPaint != null) {
        switch (wallSidesPaint) {
          case DEFAULT :
            wallSide.setColor(null);
            wallSide.setTexture(null);
            break;
          case COLORED :
            wallSide.setColor(wallSidesColor);
            wallSide.setTexture(null);
            break;
          case TEXTURED :
            wallSide.setColor(null);
            wallSide.setTexture(wallSidesTexture);
            break;
        }
      }
      if (wallSidesShininess != null) {
        wallSide.setShininess(wallSidesShininess);
      }

      if (wallSidesBaseboardVisible == Boolean.FALSE) {
          wallSide.setBaseboard(null);
      } else {
        
        Baseboard baseboard = wallSide.getBaseboard();
        float baseboardThickness = newWallBaseboardThickness;
        float baseboardHeight = newWallBaseboardHeight;
        Integer baseboardColor = null;
        HomeTexture baseboardTexture = null;
        ...
        else if ( baseboard != null) {

          baseboardThickness = baseboard.getThickness();
          baseboardHeight = baseboard.getHeight();
          baseboardColor = baseboard.getColor();
          baseboardTexture = baseboard.getTexture();
        }
        ...
        wallSide.setBaseboard(baseboard);
      }
    }
  }
...
//  }
   ...
   */
  private static final class WallSideState {
    ...
    public WallSideState(WallSide wallSide) {
      ...
      this.wallColor = wallSide.getColor();
      this.wallTexture = wallSide.getTexture();
      this.wallShininess = wallSide.getShininess();
      this.wallBaseboard = wallSide.getBaseboard();
          }
    ...
    public void reset() {
      ...
      wallSide.setColor(wallColor);
      wallSide.setTexture(wallTexture);
      wallSide.setShininess(wallShininess);
      wallSide.setBaseboard(wallBaseboard);
