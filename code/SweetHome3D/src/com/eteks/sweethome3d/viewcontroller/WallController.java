...
public class WallController implements Controller {
  ...
  protected void updateProperties() {
    ...
    if (selectedWalls.isEmpty()) {
      ...
      Integer leftSideColor = firstWall.getColor(WallSide.LEFT_SIDE);
      if (leftSideColor != null) {
        defaultColorsAndTextures = false;
        for (int i = 1; i < selectedWalls.size(); i++) {
          if (!leftSideColor.equals(selectedWalls.get(i).getColor(WallSide.LEFT_SIDE))) {
            leftSideColor = null;
            break;
          }
        }
      ...
      Integer rightSideColor = firstWall.getColor(WallSide.RIGHT_SIDE);
      if (rightSideColor != null) {
        for (int i = 1; i < selectedWalls.size(); i++) {
          if (!rightSideColor.equals(selectedWalls.get(i).getColor(WallSide.RIGHT_SIDE))) {
            rightSideColor = null;
            break;
          }
        }
      ...
      for (int i = 0; i < selectedWalls.size(); i++) {
        ...
        if (wall.getColor(WallSide.RIGHT_SIDE) != null
  ...
  public void modifyWalls() {
    ...
    if (!selectedWalls.isEmpty()) {
      ...
      WallState [] modifiedWalls = WallState.getWallStates(selectedWalls);
      // Apply modification
  ...
  private static class WallsModificationUndoableEdit extends LocalizedUndoableEdit {
    ...
    private final WallState []     modifiedWalls;
    ...
    private WallsModificationUndoableEdit(Home home,
                                          UserPreferences preferences,
                                          Selectable [] oldSelection, WallState [] modifiedWalls,
   ...
   */
  private static void doModifyWalls(WallState [] modifiedWalls, float newWallBaseboardThickness, float newWallBaseboardHeight,
                                    Float xStart, Float yStart, Float xEnd, Float yEnd,
    ...
    for (WallState modifiedWall : modifiedWalls) {
   ...
   */
  private static void undoModifyWalls(WallState [] modifiedWalls) {
    for (WallState modifiedWall : modifiedWalls) {
      Wall wall = modifiedWall.getWall();
      moveWallPoints(wall, modifiedWall.startPoint.x, modifiedWall.startPoint.y,
          modifiedWall.endPoint.x, modifiedWall.endPoint.y);
      for(int i = 0; i<2; i++) {
        
        WallSideState sideState = modifiedWall.wallSides[i];
        wall.setLeftSideColor(sideState.color);
        wall.setLeftSideTexture(sideState.texture);
        wall.setLeftSideShininess(sideState.shininess);
        wall.setLeftSideBaseboard(sideState.baseboard);
      }
