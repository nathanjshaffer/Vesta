...
public class Room extends HomeObject implements Selectable, Elevatable {
   ...
   * Creates a room from its name and the given coordinates.
   */
  public Room(Vector2D[] points) {
    this(createId("room"), Vector2D.toFloatArray(points));
  }
  ...
  public Vector2D[] getPoints() {
    return Vector2D.fromFloatArray(deepCopy(this.points));
  }
  ...
  public float getArea() {
    if (this.areaCache == null) {
      ...
      if (roomArea.isSingular()) {
        this.areaCache = Math.abs(getSignedArea(Vector2D.toFloatArray(getPoints())));
      } else {
  ...
  public boolean isClockwise() {
    return getSignedArea(Vector2D.toFloatArray(getPoints())) < 0;
  }
  ...
  public void move(float dx, float dy) {
    if (dx != 0 || dy != 0) {
      float [][] points = Vector2D.toFloatArray(getPoints());
      for (int i = 0; i < points.length; i++) {
        points [i][0] += dx;
        points [i][1] += dy;
      }
