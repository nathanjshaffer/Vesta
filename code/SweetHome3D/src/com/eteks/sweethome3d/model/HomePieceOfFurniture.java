...
public class HomePieceOfFurniture extends HomeObject implements PieceOfFurniture, Selectable, Elevatable {
  ...
  public Vector2D[] getPoints() {
    Vector2D[] piecePoints = new Vector2D[4];
  ...
  public boolean isPointAt(float x, float y, float margin) {
    for (Vector2D point : getPoints()) {
      if (Math.abs(x - point.x) <= margin && Math.abs(y - point.y) <= margin) {
        return true;
      }
    }
    return false;
  ...
  public boolean isTopLeftPointAt(float x, float y, float margin) {
    ...
    double distanceSquareToTopLeftPoint = Point2D.distanceSq(x, y, points[0].x, points[0].y);
    return distanceSquareToTopLeftPoint <= margin * margin
        && distanceSquareToTopLeftPoint < Point2D.distanceSq(x, y, points[1].x, points[1].y)
        && distanceSquareToTopLeftPoint < Point2D.distanceSq(x, y, points[3].x, points[3].y);
  }
  ...
  public boolean isTopRightPointAt(float x, float y, float margin) {
    Vector2D[] points = getPoints();
    double distanceSquareToTopRightPoint = Point2D.distanceSq(x, y, points[1].x, points[1].y);
    return distanceSquareToTopRightPoint <= margin * margin
        && distanceSquareToTopRightPoint < Point2D.distanceSq(x, y, points[0].x, points[0].y)
        && distanceSquareToTopRightPoint < Point2D.distanceSq(x, y, points[2].x, points[2].y);
  }
  ...
  public boolean isBottomLeftPointAt(float x, float y, float margin) {
    Vector2D[] points = getPoints();
    double distanceSquareToBottomLeftPoint = Point2D.distanceSq(x, y, points[3].x, points[3].y);
    return distanceSquareToBottomLeftPoint <= margin * margin
        && distanceSquareToBottomLeftPoint < Point2D.distanceSq(x, y, points[0].x, points[0].y)
        && distanceSquareToBottomLeftPoint < Point2D.distanceSq(x, y, points[2].x, points[2].y);
  }
  ...
  public boolean isBottomRightPointAt(float x, float y, float margin) {
    Vector2D[] points = getPoints();
    double distanceSquareToBottomRightPoint = Point2D.distanceSq(x, y, points[2].x, points[2].y);
    return distanceSquareToBottomRightPoint <= margin * margin
        && distanceSquareToBottomRightPoint < Point2D.distanceSq(x, y, points[1].x, points[1].y)
        && distanceSquareToBottomRightPoint < Point2D.distanceSq(x, y, points[3].x, points[3].y);
  }
