...
public class Polyline extends HomeObject implements Selectable, Elevatable {
   ...
   */
  public Polyline(Vector2D[] points) {
   ...
   * @since 6.4
   */
  public Polyline(String id, Vector2D[] points) {
   ...
   * Creates a polyline from the given coordinates.
   */
  public Polyline(Vector2D[] points, float thickness,
   ...
   * @since 6.4
   */
  public Polyline(String id, Vector2D[] points, float thickness,
                  CapStyle capStyle, JoinStyle joinStyle, DashStyle dashStyle,
                  float dashOffset, ArrowStyle startArrowStyle,
                  ArrowStyle endArrowStyle, boolean closedPath, int color) {
    super(id);
  ...
  private Vector2D[] deepCopy(Vector2D[] points) {
   ...
   */
  private void updatePoints(Vector2D[] points) {
  ...
  public void addPoint(float x, float y, int index) {
    ...
    Vector2D[] newPoints = new Vector2D [this.points.length + 1];
    System.arraycopy(this.points, 0, newPoints, 0, index);
    newPoints [index] = new Vector2D(x, y);
  ...
  public int getPointIndexAt(float x, float y, float margin) {
    for (int i = 0; i < this.points.length; i++) {
      if (Math.abs(x - this.points [i].x) <= margin && Math.abs(y - this.points [i].y) <= margin) {
        return i;
      }
    }
    return -1;
    ...
    if (this.polylinePathCache == null) {
      ...
      if (this.joinStyle == JoinStyle.CURVED) {
        for (int i = 0, n = this.closedPath ? this.points.length : this.points.length - 1; i < n; i++) {
          ...
          Vector2D previousPoint = this.points [i == 0 ?  this.points.length - 1  : i - 1];
          Vector2D point         = this.points [i];
          Vector2D nextPoint     = this.points [i == this.points.length - 1 ?  0  : i + 1];
          Vector2D vectorToBisectorPoint = nextPoint.sub(nextPoint);
          Vector2D nextNextPoint     = this.points [(i + 2) % this.points.length];
          Vector2D vectorToBisectorNextPoint = point.sub(nextNextPoint);
          curve2D.setCurve(point.x, point.y,
              point.x + (i != 0 || this.closedPath  ? vectorToBisectorPoint.x / 3.625f  : 0),
              point.x + (i != 0 || this.closedPath  ? vectorToBisectorPoint.y / 3.625f  : 0),
              nextPoint.x + (i != this.points.length - 2 || this.closedPath  ? vectorToBisectorNextPoint.x / 3.625f  : 0),
              nextPoint.y + (i != this.points.length - 2 || this.closedPath  ? vectorToBisectorNextPoint.x / 3.625f  : 0),
              nextPoint.x, nextPoint.y);
          polylinePath.append(curve2D, true);
        }
      } else {
        polylinePath.moveTo(this.points [0].x, this.points [0].y);
        for (int i = 1; i < this.points.length; i++) {
          polylinePath.lineTo(this.points [i].x, this.points [i].y);
        }
        if (this.closedPath) {
