...
public class HomeXMLExporter extends ObjectXMLExporter<Home> {
  ...
  private Map<WallPoint, String>    wallPointIds = new HashMap<WallPoint, String>();
  ...
  public void writeElement(XMLWriter writer, Home home) throws IOException {
    ...
    // Store wallPoint ids
    for (WallPoint wallPoint : home.getWallPoints()) {
      this.wallPointIds.put(wallPoint, wallPoint.getId());
    }
  ...
  protected void writeChildren(XMLWriter writer, Home home) throws IOException {
    ...
    for (WallPoint wallPoint : home.getWallPoints()) {
      writeWallPoint(writer, wallPoint);
    }
    for (Wall wall : home.getWalls()) {
   ...
   */
  protected void writeWallPoint(XMLWriter writer, WallPoint wallpoint) throws IOException {
    new ObjectXMLExporter<WallPoint>() {
        @Override
        protected void writeAttributes(XMLWriter writer, WallPoint wallpoint) throws IOException {
          writer.writeAttribute("id", wallpoint.getId());
          if (wallpoint.getLevel() != null) {
            writer.writeAttribute("level", getId(wallpoint.getLevel()));
          }
          writer.writeFloatAttribute("x", (float)wallpoint.getX());
          writer.writeFloatAttribute("y", (float)wallpoint.getY());
        }

      }.writeElement(writer, wallpoint);
  }
  ...
  protected void writeWall(XMLWriter writer, Wall wall) throws IOException {
    new ObjectXMLExporter<Wall>() {
        ...
        protected void writeAttributes(XMLWriter writer, Wall wall) throws IOException {
          ...
          writer.writeAttribute("startPoint", wall.getStartPoint().getId());
          writer.writeAttribute("endPoint", wall.getEndPoint().getId());
    ...
    new ObjectXMLExporter<Room>() {
        ...
        protected void writeChildren(XMLWriter writer, Room room) throws IOException {
          ...
          for (Vector2D point : room.getPoints()) {
            ...
            writer.writeFloatAttribute("x", point.x);
            writer.writeFloatAttribute("y", point.y);
            writer.writeEndElement();
          }
        }
  ...
  protected void writePolyline(XMLWriter writer, Polyline polyline) throws IOException {
    new ObjectXMLExporter<Polyline>() {
        ...
        protected void writeChildren(XMLWriter writer, Polyline polyline) throws IOException {
          ...
          for (Vector2D point : polyline.getPoints()) {
            writer.writeStartElement("point");
            writer.writeFloatAttribute("x", point.x);
            writer.writeFloatAttribute("y", point.y);
            writer.writeEndElement();
