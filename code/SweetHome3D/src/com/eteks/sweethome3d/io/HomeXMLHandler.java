...
public class HomeXMLHandler extends DefaultHandler {
  ...
  private final Map<String, Wall> walls = new HashMap<String, Wall>();
  private final Map<String, WallPoint> wallPoints = new HashMap<String, WallPoint>();
  private final Map<String, List<String>> wallPointIds = new HashMap<String, List<String>>();
  ...
  public void endElement(String uri, String localName, String name) throws SAXException {
    ...
    if (this.homeElementName != null && this.homeElementName.equals(name)) {
    ...
    } else if ("wallPoint".equals(name)) {
      // create wallpoint
      WallPoint wallPoint = createWallPoint(name, attributesMap);
      this.wallPoints.put(attributesMap.get("id"), wallPoint);
      this.home.addWallPoint(wallPoint);
    } else if ("wall".equals(name)) {
      Wall wall = createWall(name, attributesMap);
      this.walls.put(attributesMap.get("id"), wall);
      this.wallPointIds.put(attributesMap.get("id"), Arrays.asList(new String[]{attributesMap.get("startPoint"), attributesMap.get("endPoint")}));
      ...
      Polyline polyline = createPolyline(name, attributesMap, Vector2D.fromFloatArray(this.points.toArray(new float [this.points.size()][])));
  ...
  public void endDocument() throws SAXException {
    ...
    for (HashMap.Entry<String, List<String>> entry : this.wallPointIds.entrySet()) {
      
        Wall wall = walls.get(entry.getKey());
        wall.setStartPoint(wallPoints.get(entry.getValue().get(0)));
        wall.setEndPoint(wallPoints.get(entry.getValue().get(1)));
    }
  }
  ...
  /**
   * Returns a new {@link WallPoint} instance initialized from the given <code>attributes</code>.
   */
  private WallPoint createWallPoint(String elementName, Map<String, String> attributes) throws SAXException {
    String id = attributes.get("id");
    float x = parseFloat(attributes, "x");
    float y = parseFloat(attributes, "y");
    WallPoint wallPoint = id != null
        ? new WallPoint(id, x, y, null)
        : new WallPoint(x, y);
    return (WallPoint)resolveObject(wallPoint, elementName, attributes);
  }
  ...
  */
  private Wall createWall(String elementName, Map<String, String> attributes) throws SAXException {
   ...
   Wall wall = new Wall(id, new WallPoint(), new WallPoint(), thickness, 0, null);
